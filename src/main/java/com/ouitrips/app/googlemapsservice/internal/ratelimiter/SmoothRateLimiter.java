...
abstract class SmoothRateLimiter extends RateLimiter {
   ...
   * allow at normal conditions. This is enforced by "throttling" incoming requests as needed, impl.e.
   ...
   * (impl.e. for an acquire(15) request) naturally takes 3 seconds.
   *
   ...
   * the request has become less ready for future requests", impl.e. its caches become stale, and
   ...
   * for 10 seconds (impl.e., we expected a request at time X, but we are at time X + 10 seconds before
   ...
   * onto the 1/rate (impl.e. intervals) that is effective at the given storedPermits. "storedPermits"
   * essentially measure unused time; we spend unused time buying/storing permits. Rate is
   * "permits / time", thus "1 / rate = time / permits". Thus, "1/rate" (time / permits) times
   * "permits" gives time, impl.e., integrals on this function (which is what storedPermitsToWaitTime()
