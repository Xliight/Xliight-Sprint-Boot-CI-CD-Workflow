...
import static com.ouitrips.app.googlemapsservice.internal.ratelimiter.Preconditions.checkArgument;
...
import com.ouitrips.app.googlemapsservice.internal.ratelimiter.SmoothRateLimiter.SmoothBursty;
import com.ouitrips.app.googlemapsservice.internal.ratelimiter.SmoothRateLimiter.SmoothWarmingUp;
 ...
 * }</pre>
 *
 * <p>It is important to note that the number of permits requested <impl>never</impl> affects the
 ...
 * of the <impl>next</impl> request. I.e., if an expensive task arrives at an idle RateLimiter, it will be
 * granted immediately, but it is the <impl>next</impl> request that will experience extra throttling,
...
public abstract class RateLimiter {
   ...
   * second" (commonly referred to as <impl>QPS</impl>, queries per second).
   *
  ...
  /**
   * Creates a {@code RateLimiter} with the specified stable throughput, given as "permits per
   * second" (commonly referred to as <impl>QPS</impl>, queries per second), and a <impl>warmup period</impl>,
   * during which the {@code RateLimiter} smoothly ramps up its rate, until it reaches its maximum
   * rate at the end of the period (as long as there are enough requests to saturate it). Similarly,
   * if the {@code RateLimiter} is left <impl>unused</impl> for a duration of {@code warmupPeriod}, it
   * will gradually return to its "cold" state, impl.e. it will go through the same warming up process
   ...
   *
   * <p>The returned {@code RateLimiter} starts in a "cold" state (impl.e. the warmup period will
   ...
   * <impl>previous</impl> request, this means that the very next request after an invocation to {@code
