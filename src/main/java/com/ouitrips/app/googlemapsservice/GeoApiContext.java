...
import com.ouitrips.app.googlemapsservice.errors.OverQueryLimitException;
...
import com.ouitrips.app.googlemapsservice.internal.UrlSigner;
import com.ouitrips.app.googlemapsservice.metrics.NoOpRequestMetricsReporter;
import com.ouitrips.app.googlemapsservice.metrics.RequestMetrics;
import com.ouitrips.app.googlemapsservice.metrics.RequestMetricsReporter;
...
public class GeoApiContext implements Closeable {
  ...
  <T, R extends ApiResponse<T>> PendingResult<T> get(
    ...
    if (channel != null && !channel.isEmpty() && !params.containsKey(CHANNEL_KEY)) {
      params.put(CHANNEL_KEY, Collections.singletonList(channel));
    }
  ...
  <T, R extends ApiResponse<T>> PendingResult<T> get(
    ...
    for (int i = 0; i < params.length; i += 2) {
      if (params[i].equals(CHANNEL_KEY)) {
        channelSet = true;
      }
